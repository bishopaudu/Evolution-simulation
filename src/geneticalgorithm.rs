...
mod tests {
    ...
    fn uniform_crossover() {
       ...
       let diff_a = child.iter().zip(parent_a.iter()).filter(|(c, p)| (*c - *p).abs() > f32::EPSILON).count();
let diff_b = child.iter().zip(parent_b.iter()).filter(|(c, p)| (*c - *p).abs() > f32::EPSILON).count();
